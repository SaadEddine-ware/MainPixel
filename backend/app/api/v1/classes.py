...
async def list_classes(school_id: UUID, db: AsyncSession = Depends(get_db)):
...
async def create_class(school_id: UUID, data: ClassCreate, db: AsyncSession = Depends(get_db)):
