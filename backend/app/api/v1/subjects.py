...
async def list_subjects(school_id: UUID, level: str = None, db: AsyncSession = Depends(get_db)):
...
async def create_subject(school_id: UUID, data: SubjectCreate, db: AsyncSession = Depends(get_db)):
...
async def update_subject(subject_id: UUID, data: SubjectUpdate, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Subject).where(Subject.id == subject_id))
    obj = result.scalar_one_or_none()
    if not obj:
        raise HTTPException(status_code=404, detail="Subject not found")
